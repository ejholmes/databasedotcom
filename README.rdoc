...
= Testing
You can mock database transactions by using `Databasedotcom.mock!`. After calling this method, the gem will not make any HTTP requests
and calls to methods like `.authenticate`, `.create`, `.update`, `.upsert` and `.delete` will always succeed. The resulting record
...
`sobjects.json` within the configured fixtures directory. Then you should have an `#{sobject name}.json` file under the `describe`
folder for each object that needs to be materialized. Take a look at the `spec/fixtures/mocks` directory for examples.
